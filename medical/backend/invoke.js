...
async function invoke(data) {
	console.log('\n\n --- invoke.js - start', data.name, data.key, data.age, data.medicine);
	try {
		...
		const user = await fabric_client.getUserContext('admin', true);
		...
		// The medical chaincode is able to perform a few functions
		//   'createMed' - requires 5 args, ex: args: ['MED12', 'med', 'abc123', 'writer']
		//   'changeMedMake' - requires 2 args , ex: args: ['MED10', 'abcxyz']
		const proposal_request = {
			targets: [peer],
			chaincodeId: 'medical',
			fcn: 'createMed',
			args: [data.key, data.name, data.age, data.medicine, data.file, data.gender],
			chainId: 'mychannel',
			txId: tx_id
		};
